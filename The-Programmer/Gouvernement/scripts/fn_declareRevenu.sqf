...
if ((getNumber (missionConfigFile >> "Max_Settings_Gouvernement" >> "companies_script_enable")) isEqualTo 1) then {
    ...
    [_entreprise,((format ["%1 %2",identity_prenom,identity_nom])),_taxe,1] remoteExecCall ["max_entreprise_fnc_insertEntrepriseLogs",2];
    ...
    if (life_atmbank < _taxe) exitWith {_exit = true; hint (["STR_NOT_ENOUGHT_MONEY_BANK"] call max_gouvernement_fnc_localize);};
    life_atmbank = life_atmbank - _taxe;
...
[maxence_compte_gouv,maxence_compte_cop,maxence_compte_med,((format ["%1 %2",identity_prenom,identity_nom])),_mode,0,_taxe,3] remoteExec ["max_gouvernement_fnc_updateAccounts",2];
