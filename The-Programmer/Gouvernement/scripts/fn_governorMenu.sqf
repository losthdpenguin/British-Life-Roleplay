/*
    ...
    Owner's name:
*/
if !([((format ["%1 %2",identity_prenom,identity_nom])),(getPlayerUID player)] in maxence_gouverneur) exitWith {};
...

{
    if (!isNull _x && alive _x && !(_x isEqualTo player)) then {
        if ((side _x) == civilian) then {
            if !(([((format ["%1 %2",identity_prenom,identity_nom])),(getPlayerUID player)] in maxence_ministres) && ([((format ["%1 %2",identity_prenom,identity_nom])),(getPlayerUID player)] in maxence_security)) then {
...
lbClear _toAccount;
_transfert = [[(["STR_GOVERNEMENT_ACCOUNT"] call max_gouvernement_fnc_localize),0],[(["STR_COP_ACCOUNT"] call max_gouvernement_fnc_localize),1],[(["STR_MED_ACCOUNT"] call max_gouvernement_fnc_localize),2],[((format ["%1 %2",identity_prenom,identity_nom])),3]];
{
