        ...
        _objs = _curEntreprise getVariable ["entreprise_objects",[]];
        {
            ...
            if (_curObj isEqualType objNull) then {
                if ((typeOf _curObj) isEqualTo "Land_InfoStand_V1_F") then {
                    _curObj addAction [(["STR_SEE_COMPANY_LOGS"] call max_gouvernement_fnc_localize),{[1] spawn max_gouvernement_fnc_historicMenu;},"",0,false,false,"",'(([((format ["%1 %2",identity_prenom,identity_nom])),(getPlayerUID player)] in maxence_gouverneur) || ([((format ["%1 %2",identity_prenom,identity_nom])),(getPlayerUID player)] in maxence_ministres))'];
                };
            };
        } forEach _objs;
