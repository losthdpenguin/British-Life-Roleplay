...
*/
_classView = param [0,"",[""]];
if (_classView isEqualTo "") exitWith {};
...
[] spawn {
    for "_i" from 0 to 1 step 0 do {
        ...
        _distanceCam = 2.25 * ([boundingBoxReal _object select 0 select 0, boundingBoxReal _object select 0 select 2] distance [boundingBoxReal _object select 1 select 0, boundingBoxReal _object select 1 select 2]);
        _azimuthCam = 0;
...
if (isClass (configFile >> "CfgVehicles" >> _classView) && (getNumber (configFile >> "CfgVehicles" >> _classView >> "scope") > 0)) then {
