...
_pos = _selectedSell select 2;
_house = objectFromNetId (_selectedSell select 3);
...
life_vehicles deleteAt (life_vehicles find _house);
life_houses deleteAt (life_houses find (str(getPosATL _house)));
...
[3,_id,1,_price,(getPlayerUID player),[_className,_key,_pos],_house,((format ["%1 %2",identity_prenom,identity_nom])),_title,_description,_dureeTAX] remoteExec ["maxence_ebay_fnc_ebayAddEntry",2];
