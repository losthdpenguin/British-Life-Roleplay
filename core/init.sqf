...
if (life_HC_isActive) then {
    [getPlayerUID player,player getVariable ["realname",(format ["%1 %2",identity_prenom,identity_nom])]] remoteExec ["HC_fnc_wantedProfUpdate",HC_Life];
} else {
    [getPlayerUID player,player getVariable ["realname",(format ["%1 %2",identity_prenom,identity_nom])]] remoteExec ["life_fnc_wantedProfUpdate",RSERV];
};
...
[] spawn max_gouvernement_fnc_initGouvernement;
[player] remoteExec ["the_programmer_identity_fnc_request",2];
[] spawn The_Programmer_RepairSystem_fnc_RepairInit;
[] spawn maxence_ebay_fnc_ebayInit;
[player] remoteExec ["max_phone_fnc_cellPhoneRequest",2];
