        ...
        if (!((headgear _x) in _masks || (goggles _x) in _masks || (uniform _x) in _masks)) then {
            if (count _sPos > 1 && {_distance < 15}) then {
                _text = switch (true) do {
                    case (_x in (units group player) && playerSide isEqualTo civilian): {format ["<t color='#00FF00'>%1 %2</t>",identity_nom,identity_prenom];};

                    case (side _x isEqualTo independent): {format ["<t color='#FF0000'><img image='\textures\nhs.paa' size='1.5'></img></t> %1",_x getVariable ["realname",name _x]]};
                    default {
